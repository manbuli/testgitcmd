...
authenticate_client(request * r)
{
    ...
    char *token, *tid = NULL, *openid = NULL, *ts = NULL;
    char *extend = NULL;
    httpVar *var;
    ...
    switch (auth_response.authcode) {
        ...
        case AUTH_ALLOWED:
            ...
            debug(LOG_INFO, "req:%s", r->request.query);
            if ((var = httpdGetVariableByName(r, "tid")) != NULL) 
            {
                debug(LOG_INFO, "tid is: %s", var->value);
                tid = safe_strdup(var->value);

                if ((var = httpdGetVariableByName(r, "openId")) != NULL) 
                    openid = safe_strdup(var->value);

                if ((var = httpdGetVariableByName(r, "timestamp")) != NULL) 
                    ts = safe_strdup(var->value);

                if ((var = httpdGetVariableByName(r, "extend")) != NULL) 
                    extend = safe_strdup(var->value);

                safe_asprintf(&urlFragment, "%sgw_id=%s&tid=%s&openId=%s&timestamp=%s&extend=%s", 
                        auth_server->authserv_portal_script_path_fragment, config->gw_id,
                        tid, openid == NULL ? "" : openid, 
                        ts == NULL ? "" : ts, 
                        extend == NULL ? "" : extend);
                if(tid) 
                    free(tid);
