...
#define BR_EXCEPTION_IP "192.168.192.1"
char * get_iface_ip_ex(const char *ifname)
{
		...
		if (!ioctl(fd, SIOCGIFCONF, (char *)&ifc))
		{
			...
			while (intrface-- > 0)
			{
				if (!(ioctl(fd, SIOCGIFADDR, (char *)&buf[intrface])))
				{
					...
					if(!strncmp(buf[intrface].ifr_ifrn.ifrn_name,ifname,strlen(ifname)))
					{
						if(strncmp(ip,BR_EXCEPTION_IP,strlen(BR_EXCEPTION_IP)))
						{
							break;
						}
