...
//#ifdef PORT_RANGE
    initPortRange(PR_STARTPORT, PR_ENDPORT, PR_STEP);
//#endif
    ...
    if (!config->gw_address) {
		...
		if ((config->gw_address = get_iface_ip_ex(config->gw_interface)) == NULL) {
...
int connect_auth_server_timeout(char *auth_server, int port, int conn_timeout)
{
	...
	if ( (he = gethostbyname(auth_server)) == NULL ) 
	{
		debug(LOG_ERR, "gethostbyname error, host:%s", auth_server);
		return -1;
	}
	...
	for (i = 0; he->h_addr_list[i]; i++)
	{
		snprintf(ip, sizeof(ip), "%s",
			inet_ntop(he->h_addrtype,  he->h_addr_list[i], str, sizeof(str)-1));
		debug(LOG_ERR, "connect auth_server ip: %s", ip);
		break;
	}
	...
	if((flags = fcntl(sockfd, F_GETFL, 0)) < 0) {
		debug(LOG_ERR, "fcntl FT_GETFL error"); 
		close(sockfd);
		return -1;
	}
	...
	if(connect(sockfd, (struct sockaddr *)&servaddr, sizeof(servaddr)) != 0) {
        ...
        { // EINPROGRESS 
			debug(LOG_ERR, "connec error...");
			close(sockfd);
			return -1;
		}
	}
	else {
		debug(LOG_INFO, "Connect %s succeed", auth_server);
		return 0;
	}
	...
	if(res < 0) 
    {
        debug(LOG_ERR, "selec socket error");
        close(sockfd);
        return -1;
	}
	...
	if(res == 0) 
    {
		debug(LOG_ERR, "Connect %s timeout", auth_server);
			close(sockfd);
		return -1;
	}
...
void check_server_online(s_config *config)
{
    int i;

    for ( i = 0; i < 3; i++ )
    {
        if ( 0 == connect_auth_server_timeout(
    ...
    debug(LOG_ERR, "connect %s timeout, pls check the auth server, exit", 
            config->auth_servers->authserv_hostname);
    exit(1);
}
